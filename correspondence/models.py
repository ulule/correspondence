...
class User(Model):
    ...
    manager: Mapped["Optional[User]"] = relationship(remote_side=[id], viewonly=True)
    ...
    async def get_default_organization(
        ...
        return await self.awaitable_attrs.organization
...
class MessagePart(Model):
    ...
    message: Mapped["Optional[Message]"] = relationship(
        foreign_keys=[message_id], viewonly=True
    )
    ...
    async def send(
        ...
        if (
            ...
            if self.sender.country and self.sender.country.code in supported_countries:
                ...
                await self.conversation.aupdate(
                    asession, phone_number=from_ph, phone_numer_id=from_ph.id
                )
        else:
...
class Organization(Model):
    ...
    owner: Mapped[User] = relationship(foreign_keys=[owner_id], viewonly=True)
...
class Conversation(Model):
    ...
    last_message: Mapped[Message] = relationship(
        foreign_keys=[last_message_id], viewonly=True
    )
    ...
    phone_number: Mapped[PhoneNumber] = relationship(
        foreign_keys=[phone_number_id], viewonly=True
    )
